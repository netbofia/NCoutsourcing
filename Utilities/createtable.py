...
sqlFile=dir_path+"/../SQL/DB_schema.sql"



#Change this based on your project
variable_path="components/DB"
...
def generateTable(table,output,foreignKeys):
  ...
  for line in sql.readlines():
    ...
    if (extract and not re.match("KEY|CONSTRAINT|PRIMARY|UNIQUE",line)):
      ...
      datatype=parsed[1].replace("tinyint","TINYINT").replace("int","INTEGER").replace("varchar","STRING").replace("datetime","DATE").replace("date","DATE").replace("text","TEXT").replace("float","FLOAT").replace("enum","ENUM").replace("binary","TINYINT")
      tableAttributes[attribute]={'attribute':parsed[0],'pk':"false",'datatype':datatype,'allowNull':str(allowNull).lower(),'autoincrement':str(autoincrement).lower()}
    ...
    if (extract and re.match("PRIMARY KEY",line)):
  ...
  try:
      ...
      print("No foreignKeys on table"+table)
  result+="""
        }
      },
    });
